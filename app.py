...
FEATURES = [
    'BMI', 'PLR', 'Age (years)', 'NLR', 'UCR',
    'Urea (mmol/L)', 'Serum albumin (g/L)', 'Serum creatinine (μmol/L)',
...
SKEWED_FEATURES = [
    'Operation time (mins)', 'Serum creatinine (μmol/L)', 'Urea (mmol/L)',
    'NLR', 'PLR', 'SII'
]
...
RANGES = {
    ...
    'Serum creatinine (μmol/L)':    (36.00,  713.00),
    'UCR':                          (5.63,   42.08),
    'Serum albumin (g/L)':          (23.00,  55.00),
    'Serum potassium (mmol/L)':     (2.30,   5.70),
...
def make_shap_waterfall(model, df_processed: pd.DataFrame, raw_inputs: dict) -> bytes:
    ...
    # Build display-friendly feature names (original, unscaled values)
    surgery_inv = {v: k for k, v in SURGERY_MAP.items()}
    display_data = {}
    sanitised_features = [f.replace(' ', '_').replace('(', '_').replace(')', '_')
                            .replace('/', '_').replace('*', '_').replace('^', '_')
                            .replace('μ', '_').replace(',', '_')
                          for f in FEATURES]
    for orig, san in zip(FEATURES, sanitised_features):
        if orig == 'Surgery type':
            display_data[san] = surgery_inv.get(int(raw_inputs[orig]), str(raw_inputs[orig]))
        else:
            display_data[san] = round(raw_inputs[orig], 4)
...
st.title("🏥 Post-Surgery AKI Risk Predictor")
...
<strong>Recall&nbsp;0.529</strong> (95% CI 0.25–0.75),
<strong>Precision&nbsp;0.310</strong> (95% CI 0.122–0.6),
<strong>FPR&nbsp;0.123</strong> (95% CI 0.03–0.359).
...
with col_form:
    ...
    with st.form("prediction_form"):
        ...
        creatinine = st.number_input(
            ...
            min_value=0.0, max_value=2000.0, value=84.0, step=1.0,
            format="%.2f",
            help="Valid range: 36.00 – 713.00"
        )
        ucr = st.number_input(
        ...
        potassium = st.number_input(
            ...
            min_value=0.0, max_value=20.0, value=3.9, step=0.01,
            format="%.2f",
            help="Valid range: 2.30 – 5.70"
        )
...
with col_result:
    ...
    if not submitted:
        ...
        raw_inputs = {
            ...
            'Serum creatinine (μmol/L)':    creatinine,
            'SII':                          sii,
            'Serum potassium (mmol/L)':     potassium,
                    ...
                    if aki_pred == 1:
                        ...
                        <div class="result-positive">
                            ...
                            <div class="result-prob">
                                Predicted Probability: <strong>{prob:.4f}</strong>
                            </div>
                            <div class="result-cutoff">
                                (Threshold: {CUTOFF} | Prediction: AKI = 1)
                            </div>
                        </div>
                        """, unsafe_allow_html=True)
                    else:
                        ...
                        <div class="result-negative">
                            ...
                            <div class="result-cutoff">
                                (Threshold: {CUTOFF} | Prediction: AKI = 0)
                            </div>
                        </div>
                        """, unsafe_allow_html=True)
